...
export default function SettingsScreen() {
  ...
  const deleteFromDriveOnLocalDelete = useDriveStore((s) => s.deleteFromDriveOnLocalDelete);
  ...
  const handleGoogleConnect = async () => {
    try {
      ...
      if (ok) {
        Alert.alert('Bağlantı Başarılı 🎉', 'Google Drive hesabınız başarıyla bağlandı. Kayıtlarınızı senkronize edebilirsiniz.');
      }
    } catch (e: any) {
      Alert.alert('Bağlantı Hatası', e.message || 'Google hesabı bağlanamadı.');
    } finally {
  ...
  const handleManualDriveSync = async () => {
    ...
    try {
      ...
      if (res.success) {
        Alert.alert('Senkronizasyon Başarılı ☁️', `${res.uploadedCount} adet kayıt Google Drive'a başarıyla yedeklendi.`);
      } else {
        Alert.alert('Senkronizasyon Uyarısı', res.error || 'Yedekleme tamamlanamadı.');
      }
    } catch (e) {
  ...
  const handleExportAllZip = async () => {
    if (records.length === 0) {
      Alert.alert('Uyarı', 'Dışa aktarılacak herhangi bir kayıt bulunmuyor.');
      return;
    }
        ...
        >
          ...
          <ThemedView type="backgroundElement" style={cardStyle}>
            <View style={styles.cardHeader}>
              <MaterialIcons name="cloud-sync" size={24} color={isDriveConnected ? theme.success : theme.primary} />
              ...
              {isDriveConnected && (
                <View style={[styles.connectedPill, { backgroundColor: theme.successMuted }]}>
                  <MaterialIcons name="check-circle" size={14} color={theme.success} />
                  <ThemedText style={[styles.connectedPillText, { color: theme.success }]}>
                    Bağlı
                  </ThemedText>
                </View>
              )}
            ...
            {isDriveConnected && driveUser ? (
              <View style={styles.driveContent}>
                {/* User Info Bar */}
                <View style={[styles.userBar, { backgroundColor: theme.background, borderColor: theme.border, borderWidth: 1 }]}>
                  {driveUser.picture ? (
                    <Image source={{ uri: driveUser.picture }} style={styles.userAvatar} />
                  ) : (
                    <View style={[styles.userAvatarPlaceholder, { backgroundColor: theme.primaryMuted }]}>
                  ...
                  <Pressable
                    style={({ pressed }) => [styles.disconnectBtn, pressed && styles.buttonPressed]}
                    onPress={handleGoogleDisconnect}
                  >
                    <ThemedText style={[styles.disconnectBtnText, { color: theme.danger }]}>
                      Çıkış Yap
                    </ThemedText>
                  </Pressable>
                </View>
                ...
                <View style={styles.syncActionRow}>
                  <View style={styles.lastSyncWrapper}>
                    <ThemedText type="small" style={[styles.lastSyncLabel, { color: theme.textSecondary }]}>
                      Son Senkronizasyon:
                    </ThemedText>
                    <ThemedText type="smallBold" style={[styles.lastSyncValue, { color: theme.primary }]}>
                  ...
                  >
                    {isSyncing ? (
                      ...
                      <>
                        ...
                        <ThemedText style={styles.syncNowBtnText}>
                          Şimdi Eşitle
                        </ThemedText>
                      </>
                    )}
                ...
                {/* Toggle Settings */}
                <View style={[styles.togglesWrapper, { borderTopColor: theme.border }]}>
                  {/* Auto Sync Toggle */}
                  <View style={styles.toggleRow}>
                    <View style={styles.toggleTexts}>
                      <ThemedText type="smallBold">
                        Otomatik Senkronizasyon (Oto Upload)
                      </ThemedText>
                      <ThemedText type="small" style={[styles.toggleDesc, { color: theme.textSecondary }]}>
                        {autoSyncEnabled
              ...
              <View style={styles.driveLoginPrompt}>
                <ThemedText type="small" style={[styles.offlineFirstDesc, { color: theme.textSecondary }]}>
                  Uygulama <ThemedText type="smallBold">Offline-first (önce çevrimdışı)</ThemedText> çalışır. Tüm verileriniz cihazınızda saklanır. Google hesabınızla giriş yaparak kayıtlarınızı Google Drive'a yedekleyebilirsiniz.
                </ThemedText>
          ...
          <ThemedView type="backgroundElement" style={cardStyle}>
            <View style={styles.cardHeader}>
              <MaterialIcons name="archive" size={24} color={theme.primary} />
              <View style={styles.cardHeaderTexts}>
                <ThemedText type="smallBold" style={styles.cardTitle}>
                  Yedekleme & Dışa Aktarma (ZIP)
                </ThemedText>
                <ThemedText type="small" style={[styles.cardSubtitle, { color: theme.textSecondary }]}>
                  Tüm kayıtları, fotoğrafları ve interaktif HTML görüntüleyiciyi tek bir ZIP dosyasında paketleyin
                </ThemedText>
              </View>
            </View>
